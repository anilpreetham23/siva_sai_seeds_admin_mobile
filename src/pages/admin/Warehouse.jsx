...
import { Warehouse, Plus, X, CheckCircle, PackageSearch, Edit } from 'lucide-react';
import toast from 'react-hot-toast';
import validators from '../../utils/validators';
import FieldError from '../../components/shared/FieldError';
import { useAuth } from '../../context/AuthContext';
import WarehouseMobile from '../../components/mobile/WarehouseMobile';
...
export default function WarehouseManagement() {
  ...
  const [form, setForm] = useState({ id: null, name: '', address: '', total_capacity_kg: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, total_capacity_kg: parseFloat(form.total_capacity_kg) * 100 };
      if (form.id) {
        await warehouseService.updateWarehouse(form.id, payload);
        toast.success(t('warehouse_updated', 'Warehouse updated'));
      } else {
    ...
    } catch { toast.error(form.id ? t('failed_to_update', 'Failed to update') : t('failed_to_create') || 'Failed to create'); }
    finally { setSaving(false); }
  };

  const openEditWarehouse = (w, e) => {
    e.stopPropagation();
    setForm({ id: w.id, name: w.name, address: w.address, total_capacity_kg: (w.total_capacity_kg / 100).toString() });
    setFieldErrors({});
    setShowModal(true);
  };
    ...
    <div className="animate-fade-in">
      ...
      <div className="md:hidden">
        <WarehouseMobile
          ...
          onSelectWarehouse={(w) => setSelectedInventory(w)}
          slots={slots}
          onOpenAddWarehouse={() => {
            setForm({ id: null, name: '', address: '', total_capacity_kg: '' });
            setFieldErrors({});
            setShowModal(true);
          }}
          onOpenEditWarehouse={(w) => openEditWarehouse(w)}
          onOpenAddInv={() => setShowInvModal(true)}
          onOpenAddSlot={() => setShowSlotModal(true)}
      ...
      <div className="hidden md:block">
        <div className="page-header">
          ...
          {(user?.role === 'super_admin' || user?.role === 'admin') && (
            <button onClick={() => { setForm({ id: null, name: '', address: '', total_capacity_kg: '' }); setFieldErrors({}); setShowModal(true); }} className="btn-primary flex items-center gap-2">
                  ...
                  <div key={w.id} className={`glass-card p-5 cursor-pointer transition-all ${selectedInventory?.id === w.id ? 'ring-2 ring-primary-500' : 'hover:-translate-y-1 hover:shadow-lg'}`} onClick={() => setSelectedInventory(w)}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        {(user?.role === 'super_admin' || user?.role === 'admin') && (
                          <button onClick={(e) => openEditWarehouse(w, e)} className="p-2 rounded-xl bg-gray-50 text-gray-500 hover:text-primary-600 hover:bg-primary-50 transition-colors">
                            <Edit size={16} />
                          </button>
                        )}
                    ...
                    <div className="flex justify-between text-xs text-gray-500">
                      <span>{t("used")}: {(w.current_load_kg / 100).toFixed(0)} {t("qtl", "Qtl")}</span>
                      <span className="font-bold text-gray-800">{pct.toFixed(1)}% {t("full")}</span>
                      <span>{t("total")}: {(w.total_capacity_kg / 100).toFixed(0)} {t("qtl", "Qtl")}</span>
                    </div>
                  </div>
                );
              })
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div className="flex items-center gap-3"><div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center"><Warehouse size={20} className="text-blue-600" /></div><div><h3 className="font-bold text-gray-800">{form.id ? t('edit_warehouse', 'Edit Warehouse') : t("add_warehouse")}</h3></div></div>
